...
/*   Updated: 2021/01/28 17:11:10 by IgnacioHB        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sprite_texture(t_data *data)
{
	...
	if (ft_strchr("120NSEW",data->linea[data->i]))
		map_error();
	if ((data->linea[data->i] != ' ') && (data->sp_tex == 0)) //corregir esta linea para conseguir que de error coorecto si la S está en la primera posicion del mapa
		map_error();
	if (ft_strchr("120NSEW",data->linea[data->i]))
		map_error();
	while (data->linea[data->i] == ' ')
