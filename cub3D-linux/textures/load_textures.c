...
/*   Updated: 2021/01/26 19:41:29 by IgnacioHB        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	texture_dimensions(t_vars *vars)
{
	if (vars->side == 0 && vars->stepx == -1)
	{
		vars->tex_x = vars->norte.width;
		vars->tex_y = vars->norte.height;
	}
	else if (vars->side == 0 && vars->stepx == 1)
	{
		vars->tex_x = vars->sur.width;
		vars->tex_y = vars->sur.height;
	}
	else if (vars->side == 1 && vars->stepy == 1)
	{
		vars->tex_x = vars->este.width;
		vars->tex_y = vars->este.height;
	}
	else if (vars->side == 1 && vars->stepy == -1)
	{
		vars->tex_x = vars->oeste.width;
		vars->tex_y = vars->oeste.height;
	}
}
